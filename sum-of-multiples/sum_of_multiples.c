...
unsigned int sum(const unsigned int *factors, const size_t number_of_factors,
    ...
    entry_t *curr_entry;
    hashmap_t *map;
    unsigned int sum_acc;
    unsigned int j;

    if (factors == NULL || number_of_factors == 0) return (0);
    map = hashmap_allocate();
    for (size_t i = 0; i < number_of_factors; i++) {
        const unsigned int value = factors[i];
        if (value == 0) continue;
        j = 1;
        while ((value * j) < limit) {
            hashmap_write(map, value * j++, 0);
        }
    }
    sum_acc = 0;
    for (size_t idx = 0; idx < TABLEMAX; idx++) {
        if (map->array[idx] == NULL) continue;
        curr_entry = map->array[idx];
        while (curr_entry != NULL) {
            sum_acc += curr_entry->key;
            curr_entry = curr_entry->next;
        }
    }
